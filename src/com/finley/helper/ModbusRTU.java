...
public class ModbusRTU
{
	...
	/* CRC 高位字节值表 */
	/* CRC 高位字节值表 */
	private static byte[] MB_CRCHi = { (byte) 0x00, (byte) 0xC1, (byte) 0x81, (byte) 0x40, (byte) 0x01, (byte) 0xC0,
	...
	/* CRC低位字节值表 */
	private static byte[] MB_CRCLo = { (byte) 0x00, (byte) 0xC0, (byte) 0xC1, (byte) 0x01, (byte) 0xC3, (byte) 0x03,
	 ...
	 */
	/*
	 * public static short MB_CRC(byte []puchMsg, int spos, int usDataLen) { int
	 * uIndex ; // CRC循环中的索引 byte uchCRCHi = (byte)0xFF ; // 高CRC字节初始化 byte
	 * uchCRCLo = (byte)0xFF ; // 低CRC 字节初始化 int i = 0; while ( i<usDataLen ) //
	 * 传输消息缓冲区 { uIndex = uchCRCHi ^ puchMsg[i+spos] ; // 计算CRC uIndex &=
	 * 0x00ff; uchCRCHi = (byte)(uchCRCLo ^ MB_CRCHi[uIndex]) ; uchCRCLo =
	 * MB_CRCLo[uIndex] ; i++; } return (short)((uchCRCHi << 8) | uchCRCLo) ; }
	 */
	...
	//
	// CRC for MODBUS , probabily CRC calculation error
	public static short MBCRC(byte[] bytes, int spos, int len)
	{
		// 注意寄存器初始值是0xFFFF，
		// 网上有流传0x0000的代码，那个是不对的。
		int crc = 0xffff;

		for (int i = 0; i < len; i++)
		{
			int b = (bytes[i + spos]) & 0xff;
			crc = (crc >>> 8) ^ table[(crc ^ b) & 0xff];
		}

		return (short) (((crc >> 8) & 0XFF) | ((crc << 8) & 0XFF00));
	}

	public static void MBCRCFill(byte[] bytes, int spos, int len)
	{
		// 注意寄存器初始值是0xFFFF，
		// 网上有流传0x0000的代码，那个是不对的。
		int crc = 0xffff;
		...
		bytes[spos + len + 0] = (byte) (crc);
		bytes[spos + len + 1] = (byte) (crc >> 8);
	}
	 ...
	 * LRC校验码计算
	 */
	public static byte MBLRC(byte[] Src, int spos, int dwLen)
	{
		int i;
		int byLrc = 0;
		for (i = 0; i < dwLen; i++)
